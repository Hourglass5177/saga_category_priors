...
def render_with_max_contributor(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, filtered_mask = None):
    ...
    raster_settings = GaussianRasterizationSettingsMaxContributor(
        image_height=int(viewpoint_camera.image_height),
        image_width=int(viewpoint_camera.image_width),
        tanfovx=tanfovx,
        tanfovy=tanfovy,
    ...
    rasterizer = GaussianRasterizerMaxContributor(raster_settings=raster_settings)
...
def render_semantic_feature(viewpoint_camera, pc : FeatureGaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, norm_point_features = False):
    """
    Render the scene. 
    ...
    colors_precomp = pc.get_point_semantic_features
