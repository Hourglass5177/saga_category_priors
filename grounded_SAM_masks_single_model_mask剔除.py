...
DEFAULT_SEMANTIC_EXCLUSIONS = {
    ...
    "bed": ["person", "computer", "laptop", "book", "phone", "cup"],
    "sofa": ["person", "computer", "laptop", "book", "phone", "cup"],
}
...
def maybe_grounding_boxes(model, image_bgr: np.ndarray, class_name: str, box: np.ndarray, args) -> list[np.ndarray]:
    if model is None:
        return []
...
def semantic_exclusion_classes(class_name: str, requested: list[str] | None) -> list[str]:
    if requested:
        return [name for name in requested if name != class_name]
    return list(DEFAULT_SEMANTIC_EXCLUSIONS.get(class_name.lower(), []))


def remove_foreign_semantic_pixels(
    grounding_model,
    sam_predictor: SamPredictor,
    image_bgr: np.ndarray,
    image_rgb: np.ndarray,
    target_mask: np.ndarray,
    exclusion_classes: list[str],
    args,
    ...
    if grounding_model is None or not exclusion_classes or not target_mask.any():
        stats["reason"] = "no_model_or_exclusion_classes"
        return target_mask, stats

    detections = grounding_model.predict_with_classes(
        image=image_bgr,
        classes=exclusion_classes,
        box_threshold=args.semantic_cleanup_box_threshold,
        text_threshold=args.text_threshold,
    )
    sam_predictor.set_image(image_rgb)
    foreign_union = np.zeros_like(target_mask, dtype=bool)
    confidences = getattr(detections, "confidence", [])
    class_ids = getattr(detections, "class_id", [])
    for index, det_box in enumerate(getattr(detections, "xyxy", [])):
        class_id = class_ids[index] if index < len(class_ids) else None
        confidence = float(confidences[index]) if index < len(confidences) else 0.0
        if class_id is None or confidence < args.semantic_cleanup_box_threshold:
            continue

        masks, sam_scores, _ = sam_predictor.predict(
            box=np.asarray(det_box, dtype=np.float32),
            multimask_output=True,
        )
        best_index = int(np.argmax(sam_scores))
        foreign_mask = masks[best_index].astype(bool)
        overlap = np.logical_and(target_mask, foreign_mask)
        overlap_pixels = int(overlap.sum())
        foreign_pixels = int(foreign_mask.sum())
        overlap_ratio = overlap_pixels / max(foreign_pixels, 1)
        accepted = overlap_pixels >= args.semantic_cleanup_min_pixels and overlap_ratio >= args.semantic_cleanup_overlap
        class_name = exclusion_classes[int(class_id)] if 0 <= int(class_id) < len(exclusion_classes) else str(class_id)
        stats["detections"].append(
            {
                "class": class_name,
                "confidence": confidence,
                "sam_score": float(sam_scores[best_index]),
                "overlap_pixels": overlap_pixels,
                "foreign_overlap_ratio": float(overlap_ratio),
                "accepted": bool(accepted),
            }
        )
        if accepted:
            foreign_union |= overlap

    proposed_removed = int(foreign_union.sum())
    target_pixels = int(target_mask.sum())
    removal_ratio = proposed_removed / max(target_pixels, 1)
    stats["proposed_removed_pixels"] = proposed_removed
    stats["proposed_removal_ratio"] = float(removal_ratio)
    if removal_ratio > args.semantic_cleanup_max_removal_ratio:
        stats["reason"] = "max_removal_ratio_exceeded"
        return target_mask, stats

    cleaned_mask = np.logical_and(target_mask, ~foreign_union)
    stats["removed_pixels"] = proposed_removed
    stats["remaining_pixels"] = int(cleaned_mask.sum())
...
def parse_args():
    ...
    parser.add_argument("--semantic-cleanup", action="store_true", help="仅从现有目标 mask 中扣除检测到的其他语义像素")
    parser.add_argument("--no-semantic-cleanup", action="store_false", dest="semantic_cleanup")
    parser.add_argument("--semantic-exclusion-classes", nargs="+", default=None, help="需要从目标 mask 中剔除的类别；默认按目标类别选择")
    parser.add_argument("--semantic-cleanup-box-threshold", type=float, default=0.40)
    parser.add_argument("--semantic-cleanup-overlap", type=float, default=0.50, help="异类 mask 落入目标 mask 的最低比例")
    parser.add_argument("--semantic-cleanup-min-pixels", type=int, default=32)
    parser.add_argument("--semantic-cleanup-max-removal-ratio", type=float, default=0.45, help="单张图最多允许删除的目标 mask 比例")
    ...
    parser.set_defaults(constrain_to_3d_support=False, semantic_cleanup=True)
...
def main():
    ...
    exclusion_classes = semantic_exclusion_classes(class_name, args.semantic_exclusion_classes)
    ...
    if args.mode in ("grounded_sam", "hybrid") or (args.semantic_cleanup and exclusion_classes):
        if Model is None:
    ...
    for index, candidate in enumerate(tqdm(view_candidates)):
        ...
        if args.mode in ("grounded_sam", "hybrid"):
            candidate_boxes.extend(maybe_grounding_boxes(grounding_model, image_bgr, class_name, projection_box, args))
        ...
        if args.semantic_cleanup:
            final_mask, cleanup_stats = remove_foreign_semantic_pixels(
                grounding_model,
                sam_predictor,
                image_bgr,
                image_rgb,
            ...
            stats["semantic_cleanup"] = cleanup_stats
        else:
            stats["semantic_cleanup"] = {"enabled": False, "removed_pixels": 0}
    ...
    summary = {
        ...
        "mode": args.mode,
        "semantic_cleanup": bool(args.semantic_cleanup),
        "semantic_exclusion_classes": exclusion_classes,
