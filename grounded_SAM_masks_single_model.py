...
def visible_target_points_by_depth(
    ...
    if len(all_z) == 0:
        return np.zeros((0, 2), dtype=np.int32), np.zeros(0, dtype=np.float64), 0.0
    ...
    depths = tgt_z[visible]
    return points_2d, depths, float(len(points_2d)) / max(tgt_valid_count, 1)


def target_visible_projection(
    all_points_xyz: np.ndarray,
    target_points_xyz: np.ndarray,
    camera: dict,
    image_shape: tuple[int, int],
    depth_tolerance: float,
    use_depth_check: bool,
):
    """统一返回可见目标点的 2D 坐标、相机深度和可见比例。"""
    if use_depth_check:
        return visible_target_points_by_depth(
            all_points_xyz,
            target_points_xyz,
            camera,
            image_shape,
    ...
    if len(tgt_z) == 0:
        return np.zeros((0, 2), dtype=np.int32), np.zeros(0, dtype=np.float64), 0.0
    points_2d = np.stack([tgt_u, tgt_v], axis=1)
    return points_2d, tgt_z, float(len(points_2d)) / max(tgt_valid_count, 1)


def filter_depth_coherent_points(
    points_2d: np.ndarray,
    depths: np.ndarray | None,
    mad_scale: float = 2.5,
) -> tuple[np.ndarray, np.ndarray | None]:
    """剔除深度离群点，保留同一可见表面上的点。"""
    if depths is None or len(depths) != len(points_2d) or len(points_2d) == 0:
        return points_2d, depths
    med = np.median(depths)
    mad = np.median(np.abs(depths - med)) + 1e-6
    keep = np.abs(depths - med) <= mad_scale * mad
    if not np.any(keep):
        return points_2d, depths
    return points_2d[keep], depths[keep]
...
def sample_positive_points(
    ...
    depths: np.ndarray | None,
    count: int,
    depth_mad_scale: float = 2.5,
    cluster_percentile: float = 70.0,
) -> np.ndarray:
    """在可见、深度一致且空间密集的区域采样彼此距离较近的正点。"""
    points_2d, _ = filter_depth_coherent_points(points_2d, depths, depth_mad_scale)
    if len(points_2d) == 0:
    ...
    dist2d = np.linalg.norm(points_2d - center, axis=1)
    radius = np.percentile(dist2d, cluster_percentile)
    core_mask = dist2d <= max(radius, 4.0)
    if core_mask.sum() < max(count, 3):
        core_mask = np.ones(len(points_2d), dtype=bool)
    core_pts = points_2d[core_mask]
    if len(core_pts) <= count:
        return core_pts.astype(np.float32)

    core_center = np.median(core_pts, axis=0)
    start = int(np.argmin(np.linalg.norm(core_pts - core_center, axis=1)))
    selected = [start]
    for _ in range(1, count):
        last = core_pts[selected[-1]]
        dists = np.linalg.norm(core_pts - last, axis=1)
        dists[selected] = np.inf
        selected.append(int(np.argmin(dists)))
    return core_pts[selected].astype(np.float32)
...
def sample_negative_points(
    ...
    count: int,
    margin: int = 16,
    pos_points: np.ndarray | None = None,
) -> np.ndarray:
    h, w = image_shape
    ...
    if pos_points is not None and len(pos_points) > 0:
        pos_center = np.median(pos_points, axis=0)
        order = np.argsort(np.linalg.norm(pts - pos_center[None, :], axis=1))[::-1]
...
def sample_internal_negative_points(
    ...
    pos_points: np.ndarray | None = None,
    grid: int = 8,
    min_dist_from_pos: float = 12.0,
) -> np.ndarray:
    """在投影 box 内采样没有目标点支持、且远离正点的位置，作为 SAM 负点。"""
    if count <= 0:
        return np.zeros((0, 2), dtype=np.float32)
    ...
    pts = np.asarray(candidates, dtype=np.float32)
    if pos_points is not None and len(pos_points) > 0:
        pos = np.asarray(pos_points, dtype=np.float32)
        min_dists = np.min(np.linalg.norm(pts[:, None, :] - pos[None, :, :], axis=2), axis=1)
        valid = min_dists >= min_dist_from_pos
        if np.any(valid):
            pts = pts[valid]
            min_dists = min_dists[valid]
...
def parse_args():
    ...
    parser.add_argument(
        "--positive-depth-mad-scale",
        type=float,
        default=2.5,
        help="正点采样时剔除深度离群点的 MAD 倍数",
    )
    parser.add_argument(
        "--positive-cluster-percentile",
        type=float,
        default=70.0,
        help="正点采样时保留的 2D 密集核心百分位半径",
    )
    parser.add_argument(
        "--neg-min-dist-from-pos",
        type=float,
        default=12.0,
        help="内负点与正点的最小 2D 像素距离",
    )
...
def main():
    ...
    for stem, camera in tqdm(image_items):
        ...
        if args.no_depth_occlusion_check:
            points_2d, points_depth, visible_ratio = target_visible_projection(
                points_xyz,
                instance_points,
                camera,
                (h, w),
    ...
    for index, candidate in enumerate(tqdm(view_candidates)):
        ...
        points_depth = candidate.get("points_depth")
        ...
        pos_points = sample_positive_points(
            points_2d,
            points_depth,
            args.positive_points,
            depth_mad_scale=args.positive_depth_mad_scale,
            cluster_percentile=args.positive_cluster_percentile,
        )
        outer_neg_points = sample_negative_points(
            projection_box,
            (h, w),
        ...
        inner_neg_points = sample_internal_negative_points(
            ...
            pos_points=pos_points,
            min_dist_from_pos=args.neg_min_dist_from_pos,
        )
