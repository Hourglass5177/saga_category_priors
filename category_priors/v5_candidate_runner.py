...
from .v5_candidate import SOURCES, validate_source


def v5_candidate_run_paths(
    output_root: str | Path, source: str, scene_id: str, seed: int,
) -> dict[str, Path]:
    validate_source(source)
    run_dir = Path(output_root).resolve() / source / scene_id / f"seed-{int(seed)}"
    return {
        "run_dir": run_dir,
        ...
        "proposals": run_dir / "v5_proposals.json",
        "proposal_labels": run_dir / "v5_proposal_labels.npz",
...
def build_v5_candidate_command(
    *, pipeline: str | Path, scene: Mapping[str, Any], output_root: str | Path,
    source: str, scene_id: str, seed: int, git_commit: str,
) -> tuple[list[str], dict[str, Path]]:
    paths = v5_candidate_run_paths(output_root, source, scene_id, seed)
    command = [
        ...
        "--minimal-metadata", "--v5-candidate-source", source,
        "--v5-candidate-output", str(paths["proposals"]),
        "--v5-candidate-labels-output", str(paths["proposal_labels"]),
        "--v5-git-commit", str(git_commit), "--v5-scene-id", str(scene_id),
    ]
...
def _complete(paths: Mapping[str, Path], source: str, command: Sequence[str] | None = None) -> bool:
    ...
    try:
        ...
        return False
    if proposal.get("kind") != "v5_proposal_bank" or proposal.get("source") != source:
        return False
    if command is not None:
        try:
            ...
            return False
        if runner.get("kind") != "v5_candidate_run" or runner.get("command") != list(command):
            return False
    return True


def execute_v5_candidate_runs(
    *, scene_manifest: str | Path, output_root: str | Path, pipeline: str | Path,
    git_commit: str, scene_ids: Sequence[str], sources: Sequence[str] = SOURCES,
    seeds: Sequence[int] = (42,), resume: bool = True, continue_on_error: bool = False,
    dry_run: bool = False, max_runs: int | None = None,
) -> dict[str, Any]:
    scenes = load_scene_runtime_manifest(scene_manifest)
    requested_sources = [validate_source(str(value)) for value in sources]
    if not requested_sources:
        raise ValueError("at least one V5 source is required")
    runs = [
        (source, str(scene_id), int(seed))
        for scene_id in scene_ids for source in requested_sources for seed in seeds
    ]
    ...
    for source, scene_id, seed in runs:
        command, paths = build_v5_candidate_command(
            pipeline=pipeline, scene=scenes[scene_id], output_root=output_root,
            source=source, scene_id=scene_id, seed=seed, git_commit=git_commit,
        )
        record = {"source": source, "scene_id": scene_id, "seed": seed, "run_dir": str(paths["run_dir"])}
        if resume and _complete(paths, source, command):
        ...
        status = "complete" if completed.returncode == 0 and _complete(paths, source) else "failed"
        write_json(paths["runner"], {
            "kind": "v5_candidate_run", "git_commit": str(git_commit),
            "source": source, "scene_id": scene_id, "seed": seed, "status": status,
            "runtime_seconds": runtime, "return_code": completed.returncode, "command": command,
        })
        if status == "complete" and not _complete(paths, source, command):
            status = "failed"
        ...
        if status == "failed" and not continue_on_error:
            raise RuntimeError(f"V5 candidate run failed: {source}/{scene_id}/seed-{seed}")
    return {
        "kind": "v5_candidate_execution", "git_commit": str(git_commit), "total": len(records),
