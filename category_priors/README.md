...
当前研究问题是：老师兼容自动流程产生全 SAGA20 分支候选后，把最终真实导出的分支候选投回最清楚的二维视角，用 GroundingDINO 与 SAM 做一次复核，是否能减少假实例；按预测类别的典型物理尺寸决定裁图范围，是否比全类别共用一个尺寸更好。
...
正式比较固定为 `B0/raw/global/class`，只运行 DEV8。不得下载或训练，不得重跑候选生成，不得通过保护式 KNN、事后插回或新对象主干改变候选几何。
