...
随后在真实输出中进一步发现，原逻辑还会对背景 label `-1` 做最终 2D 类别 vote；当背景
vote 误胜为某个 `selected_class` 时，`instances["-1"]` 会被写出（`scene0064_01` 中为
`cabinet`）。负 ID 在点标签协议中始终表示背景，绝不能成为预测实例。旧评价适配器若遍历
