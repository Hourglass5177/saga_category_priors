...
def _matching_summary(
    *,
    ...
    subset_masks = [masks[index] for index in subset]
    matrix = _iou_matrix(subset_masks, gt_objects)
...
def evaluate_candidate_set_three_spaces(
    *,
    ...
    formal_masks = [
        formal_gt_point_mask(value.gaussian_ids, nearest) for value in normalized
    ]
    ...
    for candidate_index, candidate in enumerate(normalized):
        formal_mask = formal_masks[candidate_index]
        geometry_iou, geometry_gt, _ = _best_match(formal_mask, official_gt)
        same_iou, same_gt, _ = _best_match(
            formal_mask, official_gt, class_id=candidate.class_id
        )
        _, target, _ = _best_match(
            formal_mask, ground_truth, class_id=candidate.class_id
        )
        ids = _normalize_ids(
        ...
        candidate_rows.append(
            {
                ...
                "formal_gt_point_count_5cm": int(np.count_nonzero(formal_mask)),
                "official_evaluable": int(np.count_nonzero(formal_mask))
                >= int(min_region_size),
