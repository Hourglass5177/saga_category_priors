# SAGA 类别先验与小物体保护实验计划（当前权威：候选形成、KNN吞噬与先验域三分诊断）

> **权威状态：第28节的全类别类别化降噪已在开发8场停止；当前执行第29节的三分诊断。
> 这轮诊断不调阈值、不生成新候选，也不把使用GT的机械实验包装成可部署方法。**
...
- 当前状态：**第28节DEV8已停止；一般类别先验尚未被否定，正在执行第29节三分根因诊断**
...
- [ ] 提交并部署独立云端工作区；
- [ ] 在冻结8场运行E1、E2、E3并核对1344/15/4等输入身份；
- [ ] 根据三项结果更新根因结论，不调阈值、不修改候选。
