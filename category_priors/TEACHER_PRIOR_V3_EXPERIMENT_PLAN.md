...
本轮技术验证全部通过。`A1B1`在实例重编号后与冻结输出的点标签、类别和实例数一致；正式
评价继续使用冻结输出中的原始分数。重放计算出的少数分数与冻结值最多相差一个64位浮点数
的最小表示单位，这一差异已单独记录，不改变分数排序、正式AP或任何科学门槛。

H′在晚期过滤之前共有9803个accepted components，其中38个没有可用于几何评价的full支持，
剩余9765个组件中只有5个能与GT形成一对一的IoU≥0.50匹配，低于预注册要求的6个。也就是
说，正确完整对象的数量在进入晚期过滤之前就已经不足，单靠放宽后端删除规则无法达到本节的
最低容量要求。
...
关闭逐点检测硬过滤能够找回一些IoU较低的支持，并提高小物体召回，但没有增加IoU≥0.50的
完整对象。把两类晚期删除同时放宽后，IoU≥0.50匹配从1个增加到3个，却同时产生190个候选，
Precision@0.25降至7.37%，仍未达到至少6个完整对象和10%准确率的门槛。这说明晚期过滤确实
会删除部分有用支持，但它不是当前实例质量不足的主要根因；全面放宽只会同时释放更多碎片和
假阳性。

P-flat敏感性结果与这一判断一致。其accepted components中也只有5个一对一IoU≥0.50匹配；
最宽松的`A0B0`最终得到4个IoU≥0.50匹配，仍达不到6个的绝对门槛。P没有单独通过科学门槛，
也不能覆盖H′的正式结论。

因此，本节按预注册规则停止：不实施条件性清洁导出器，不扩展DEV8，也不运行类别先验。这里
不能写“类别先验无效”，因为类别先验从未进入本轮bank、关联、剪枝或评价。当前能够确认的
是：现有accepted components及其上游提供的完整对象数量不足，晚期过滤虽然造成了一部分损失，
但放宽它无法把碎片恢复成足够多的完整实例。

如果继续研究，下一条路线应改变accepted components之前的对象证据，而不是继续调整晚期阈值。
可选方向是使用真正对象级的二维掩码，或采用SAI3D式几何超点建立对象观察。新的无先验主干
先达到健康门槛后，才有资格在同一候选和身份边上比较全局尺寸约束与逐类别尺寸约束。
