...
> **权威状态：第35节两步闭环已经完成并按机械门槛停止。冻结 DEV8 的评价纠正和阶段
> 重建全部通过；同源 H′/P 两场正控中，P 消除了层级歧义并提高候选准确率，但没有提高
> 完整对象数量、IoU≥0.50 匹配或小物体召回。`scene0025_01` 的完整 P 路径重复运行还出现
> 1 条离散 Gaussian 成员差异，因此科学门槛按预注册规则不具备正式判定资格。类别先验
> 本轮没有测试，不能据此宣布有效或无效。**
...
- 当前状态：**第35节已因完整 P 重复合同失败而停止；类别先验尚未进入本轮**
...
第24至33节证明了若干旧接口失败，但没有建立一条健康、独立、完整掩码优先的自动实例基线。老师原型确实使用二维掩码；准确的结构缺口是，掩码信息在训练后被间接压缩进关联和语义特征，但精确的逐帧掩码成员关系及跨视角对应关系没有作为运行时对象图持续保留，自动实例仍由 HDBSCAN、中心分配和全场近邻重新形成。不能再把“旧基线用了掩码”和“旧基线直接以完整掩码形成跨视角对象”混为一谈。
...
本节严格区分“语义类别”和“物体身份”。前者回答物体是椅子、书还是桌子，后者回答它是左边第一把椅子还是右边第二把椅子。语义特征不能区分同类别实例；关联特征由二维掩码共现监督得到，只表示相似程度，也不是明确的跨视角物体编号。

老师原代码并非没有使用二维掩码。它用掩码训练语义特征和关联特征，但自动后处理先按语义选取同类 Gaussian，再用关联、空间和语义混合距离做 HDBSCAN，随后还有中心全点重分配、全场近邻和小簇过滤。因此准确表述是：二维掩码信息被间接压缩进特征，但精确的逐帧掩码成员关系及跨视角对应关系没有作为运行时对象图持续保留；不得写成“二维掩码身份被完全丢弃”。

清洁结构不是简单换一个聚类器，而是把聚类单位从单个 Gaussian 改为“每帧完整二维掩码的一次物体观察”。当前 H′/P 两步只允许用共同 Gaussian、相机可见性和多视角一致性形成无先验对象图；语义只在对象冻结后分类，GT只进入评价。关联特征将来至多补充模糊的已有联系，不能充当物体编号。

类别先验将来应能约束合并、分裂和保留，而不只是实例形成后的降噪；例如按预测类别尺寸否决超大合并，或按合理支持量保护小实例。但类别先验只能约束已经存在的身份边，不能仅凭“都是椅子”生成身份边。当前两步实验仍保持单因素并精度优先报告每一级的一对一 IoU 匹配、候选准确率、污染和假阳性；只有无先验实例主干达到最低健康水平后，才在同一原始掩码图和身份边上比较全局约束与逐类约束。在此之前不得宣布类别先验无效。

RTX 5090 预检确认，透明度质量通过 CUDA 原子加法归约时，未被后续算法使用的浮点末位会随线程调度轻微变化；同一真实帧的可见 Gaussian、掩码成员、平面化唯一所有权、歧义成员和类别分布均保持完全一致，三类冻结阈值也没有发生穿越。新生产合同因此先用原始质量完成全部阈值和所有权判断，再只持久化确定性的成员指示值；连续透明度质量不进入银行文件，也不得被解释为被测质量。完整 P 路径的独立重复仍是硬门槛：只要任何离散成员或最终输出不同，实验即按技术失败停止，不能用该合同掩盖。
...
停止后删除当前任务的每小时检查自动化。用户随后明确授权：确认本任务终态、产物安全且不再
需要云端后，可自行关闭AutoDL“三维重建”实例，无需再次确认。

- [x] 用户批准两步一次性实施，科学得分不在两步之间触发询问；
- [x] 主代理完整回读权威文档并冻结本节边界；
- [x] 实现三空间评价、一对一匹配、双AP协议和阶段重建；
- [x] 实现同源SAM元数据、H′/P派生和Gaussian唯一观察合同；
- [x] 本地专项305项、仓库首方1070项通过（另2项按既有条件跳过）；
- [x] 生产代码以`d3c7814`提交并部署到`clean-mask-contract-d3c7814`；
- [x] 第一步全部技术门槛通过，冻结DEV8评价和最终分区重建验收完成；
- [x] 两场H′/P完成；因`scene0025_01`完整P重复运行出现1条离散成员差异，按硬门槛停止；
- [x] `89a38b0`修复PLY内容身份评价器字段合同，并用旧产物只读复评；未重跑SAM、lifting或共识。

### 35.6 实际结果与终态（2026-09-02）

第一步的评价纠正是成功的。GT-as-prediction、官方九档AP、历史十档AP、GT到Gaussian映射、
冻结输出重建以及旧产物只读身份检查全部通过。新的正式指标将GT点域、Gaussian到GT精度和
GT到Gaussian覆盖分开计算，不再把密集Gaussian伪装成额外假阳性。阶段漏斗也能逐级重建到
与冻结`output.json`相同的最终分区。

初次复评曾报告四套H′/P bank的PLY内容身份均不匹配。只读法证确认生产端登记的是
`file_count/total_bytes/relative_paths/manifest_sha256`合同，而评价器误按不存在的
`size/sha256`字段读取；这是假告警，不是资产漂移。`89a38b0`改为复用生产端同一个内容身份
函数后，四套bank的PLY身份、原始PLY乘场景尺度后的XYZ和清单路径全部精确匹配。云端专项测试
20项、本地清洁基线测试344项及仓库首方测试1103项通过（另2项按既有条件跳过）。

真正的机械失败只发生在`scene0025_01`的P-flat完整重复路径。差异定位到第6帧
`frame-000120`、全局mask 222、Gaussian 441776：重复bank相对主bank只多这一条稀疏支持成员，
其余支持数组和`masks.json`完全一致。该点位于`inside_mass≥0.5`或
`inside_mass/visible_mass≥0.5`的临界判断附近，CUDA原子归约末位变化造成离散阈值穿越。
最终`point_labels`和实例声明没有变化，但bank及完整条件目录不再逐字节相同。按照35.0的硬
门槛，本轮必须记为`mechanical-contract-failed`；不得事后加epsilon、量化或容差把当前结果
改写成通过。`scene0645_00`的bank和最终条件目录则完整逐字节重复通过。

以下科学数值只作为机制观察，不构成通过结论。两场汇总中，正式可评候选从H′的42个降到P的
20个；一对一IoU≥0.25匹配从6个降到5个，但precision由0.1429升到0.2500；IoU≥0.50两边都
只有1个匹配，P没有新增完整对象。tiny/small Recall@0.25由0.1429降到0.0952。Gaussian几何
precision由0.2584升到0.3647，污染率由0.7416降到0.6353，说明平面化确实删掉了一批错误
支持，却也删掉了真实支持。阶段漏斗显示更大的共同损失位于“accepted components→检测比例
过滤”（两臂、两场均删除约97%的对象），唯一所有权到最终导出又删除约59%至73%。因此不能把
失败只归因于分层歧义：当前完整掩码的对象完整性、alpha提升后的支持，以及晚期检测过滤/导出
共同限制了自动实例质量。

预注册科学门槛即使忽略机械失败也未通过：P相对H′新增IoU≥0.50匹配为0（要求至少2），累计
只有1个（要求至少6），tiny/small Recall@0.25为0.0952（要求至少0.20），没有场景方向为正；
`scene0645_00`的Recall@0.25下降0.0526，略超过允许的0.05。结论只能是：平面唯一观察提高了
准确率、减少了污染，但没有恢复完整对象，不能扩到DEV8。类别先验没有进入bank、共识、分类或
评价，本轮完全没有检验类别先验。

本轮终止后不再调整共识阈值。若继续研究，新的授权路线只能比较真正对象级的二维掩码来源，
...
- [x] 实现纯重放、评价、CLI和回归测试；
- [x] 本地测试、commit/push后才启动云端；
- [x] H′与P的晚期2×2重放已完成，技术合同全部通过；
- [x] H′没有实验臂通过能力门槛，条件性清洁导出器未获授权，未扩展DEV8；
- [x] 本节按预注册规则停止，类别先验没有进入实验，因此尚未被检验。

### 36.5 实际结果与停止结论

本轮技术验证全部通过。`A1B1`在实例重编号后与冻结输出的点标签、类别和实例数一致；正式评价继续使用冻结输出中的原始分数。重放计算出的少数分数与冻结值最多相差一个64位浮点数的最小表示单位，这一差异已单独记录，不改变分数排序、正式AP或任何科学门槛。

H′在晚期过滤之前共有9803个accepted components，其中38个没有可用于几何评价的full支持，剩余9765个组件中只有5个能与GT形成一对一的IoU≥0.50匹配，低于预注册要求的6个。也就是说，正确完整对象的数量在进入晚期过滤之前就已经不足，单靠放宽后端删除规则无法达到本节的最低容量要求。

四个H′实验臂的结果如下：

| 条件 | 候选数 | IoU≥0.25匹配 | Precision@0.25 | IoU≥0.50匹配 | tiny/small Recall@0.25 |
|---|---:|---:|---:|---:|---:|
| `A1B1-current` | 42 | 6 | 14.29% | 1 | 14.29% |
| `A0B1-no-detection-hard-filter` | 56 | 12 | 21.43% | 1 | 23.81% |
| `A1B0-pre-late-pruning` | 93 | 7 | 7.53% | 1 | 19.05% |
| `A0B0-both-relaxed` | 190 | 14 | 7.37% | 3 | 28.57% |

关闭逐点检测硬过滤能够找回一些IoU较低的支持，并提高小物体召回，但没有增加IoU≥0.50的完整对象。把两类晚期删除同时放宽后，IoU≥0.50匹配从1个增加到3个，却同时产生190个候选，Precision@0.25降至7.37%，仍未达到至少6个完整对象和10%准确率的门槛。这说明晚期过滤确实会删除部分有用支持，但它不是当前实例质量不足的主要根因；全面放宽只会同时释放更多碎片和假阳性。

P-flat敏感性结果与这一判断一致。其accepted components中也只有5个一对一IoU≥0.50匹配；最宽松的`A0B0`最终得到4个IoU≥0.50匹配，仍达不到6个的绝对门槛。P没有单独通过科学门槛，也不能覆盖H′的正式结论。

因此，本节按预注册规则停止：不实施条件性清洁导出器，不扩展DEV8，也不运行类别先验。这里不能写“类别先验无效”，因为类别先验从未进入本轮bank、关联、剪枝或评价。当前能够确认的是：现有accepted components及其上游提供的完整对象数量不足，晚期过滤虽然造成了一部分损失，但放宽它无法把碎片恢复成足够多的完整实例。

如果继续研究，下一条路线应改变accepted components之前的对象证据，而不是继续调整晚期阈值。可选方向是使用真正对象级的二维掩码，或采用SAI3D式几何超点建立对象观察。新的无先验主干先达到健康门槛后，才有资格在同一候选和身份边上比较全局尺寸约束与逐类别尺寸约束。

## 37. 研究范围纠正与全类别尺寸先验最小闭环

2026-09-03 的任务回溯确认：老师交付的 `postprocess.py` 原本就是自动实例流程，自动实例任务不是后来增加的；`other_classes` 只是 28 标签/7 小物类别交付候选中的启发式支线，不是研究的全部范围。当前兼容实验实际使用 32 标签/8 小物类别流程，并在 SAGA20 上评价。公开 SAGA 仍只作为提示式方法来源，不能称为这里的自动实例 AP 基线。

第 24–36 节记录的结构审计和失败结论继续保留，但它们不能代替最初科学问题的直接检验，也不能支持“类别先验整体无效”。下一项登记实验不再重建候选主干：冻结修正后老师自动流程在 `merged_partition` 形成的全类别 KNN 前临时实例，只比较 global 与 predicted-class 的 train-only 米制三轴尺寸统计。公共分数仅使用可跨 global/branch 来源比较的 KNN 前二维投票比例；特殊分支覆盖标签后仍错位的旧全局 assignment confidence 禁止进入实验。

完整的候选合同、严格一对一评价、正式 0.50–0.90 九档协议、停止门槛和独立复核边界，以 [FULL_INSTANCE_SIZE_PRIOR_EXPERIMENT_PLAN.md](FULL_INSTANCE_SIZE_PRIOR_EXPERIMENT_PLAN.md) 为准。网页审计的收束意见及仓库核对纠正见 [WEB_AUDIT_SUMMARY_20260903.md](WEB_AUDIT_SUMMARY_20260903.md)。

### 37.1 当前执行检查点（2026-09-03）

- [x] 将网页端原始审计报告逐字纳入仓库，并把本地核查后的纠正单独记录；
- [x] 实现唯一入口 `run-full-instance-size-prior`，不向旧 `postprocess.py` 增加实验分支；
- [x] 实现 `merged_partition` 全实例快照、KNN 前 33 路投票、同池 global/class 尺寸评分、严格一对一候选评价、条件性恢复和分阶段自动停止；
- [x] 空恢复从 `post_filter` 真实重放最终投票与输出合同，逐点标签、类别、包围盒、实例数及原生分数必须与冻结 T1-B1 相同；
- [x] 强化 T1 复用身份：完整命令只允许解释器、生产脚本及四个输出路径占位，producer 只允许登记的 V9 历史提交或当前提交，并核验生产脚本 Git blob 与四项产物 SHA256；
- [x] 缺失 T1 重建固定使用启动器已通过 CUDA 预检的同一 Python，单 GPU 固定为 `CUDA_VISIBLE_DEVICES=0`；
- [x] 运行清单若声明内容哈希则强制核验，split audit 记录 tune/final GT、清单、尺寸分层和训练统计的真实内容身份；各场景边界重复执行 80 GiB 磁盘守卫并读取 cgroup 内存状态；
- [x] 候选集合的阈值 F1 使用“最大匹配数、再最大总 IoU”的集合匹配；按分数顺序的一对一贪心只用于 AP，二者不混用；
- [x] 专项测试通过；仓库 `tests/` 首方回归共 1195 项通过、2 项按既有条件跳过，Python 编译和 shell 语法检查通过；
- [ ] commit/push 后部署到提交绑定的新云端工作区；
- [ ] Stage 0/1 技术合同通过后运行 DEV8 候选容量与排序；只有逐级通过时才进入恢复、holdout、tune24 和 final48。
