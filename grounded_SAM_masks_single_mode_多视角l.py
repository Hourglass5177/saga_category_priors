...
def project_points_indexed(points_xyz: np.ndarray, camera: dict, image_shape: tuple[int, int]):
    """投影点并保留输入点索引关系，供多视角一致性投票使用。"""
    h, w = image_shape
    pts_h = np.concatenate([points_xyz, np.ones((len(points_xyz), 1), dtype=np.float64)], axis=1)
    cam_pts = (camera["w2c"] @ pts_h.T).T[:, :3]
    z = cam_pts[:, 2]
    valid = z > 1e-3
    u = np.zeros(len(points_xyz), dtype=np.int32)
    v = np.zeros(len(points_xyz), dtype=np.int32)
    if np.any(valid):
        u_float = camera["fx"] * cam_pts[valid, 0] / z[valid] + camera["cx"]
        v_float = camera["fy"] * cam_pts[valid, 1] / z[valid] + camera["cy"]
        u[valid] = np.round(u_float).astype(np.int32)
        v[valid] = np.round(v_float).astype(np.int32)
    valid &= (u >= 0) & (u < w) & (v >= 0) & (v < h)
    return u, v, z, valid


def visible_scene_projection(
    points_xyz: np.ndarray,
    camera: dict,
    image_shape: tuple[int, int],
    depth_tolerance: float,
):
    """返回完整场景点的投影，以及经过每像素深度竞争后的可见点掩码。"""
    h, w = image_shape
    u, v, z, valid = project_points_indexed(points_xyz, camera, image_shape)
    valid_ids = np.flatnonzero(valid)
    visible = np.zeros(len(points_xyz), dtype=bool)
    if len(valid_ids) == 0:
        return u, v, z, visible
    flat = v[valid_ids] * w + u[valid_ids]
    depth_map = np.full(h * w, np.inf, dtype=np.float64)
    np.minimum.at(depth_map, flat, z[valid_ids])
    visible[valid_ids] = z[valid_ids] <= depth_map[flat] + depth_tolerance
    return u, v, z, visible


def build_multiview_consistency(
    points_xyz: np.ndarray,
    point_mask: np.ndarray,
    first_pass_views: list[dict],
    args,
):
    """将第一遍二维 mask 投票回3D，找出稳定落入目标 mask 的非目标场景点。"""
    visible_count = np.zeros(len(points_xyz), dtype=np.uint16)
    inside_count = np.zeros(len(points_xyz), dtype=np.uint16)
    for view in tqdm(first_pass_views, desc="多视角3D投票"):
        mask = view["first_mask"]
        u, v, _, visible = visible_scene_projection(
            points_xyz,
            view["camera"],
            mask.shape,
            args.multiview_depth_tolerance,
        )
        ids = np.flatnonzero(visible)
        if len(ids) == 0:
            continue
        visible_count[ids] += 1
        inside_count[ids] += mask[v[ids], u[ids]].astype(np.uint16)

    consistency = inside_count.astype(np.float32) / np.maximum(visible_count, 1)
    foreign_points = (
        ~point_mask
        & (visible_count >= args.multiview_min_views)
        & (consistency >= args.multiview_foreign_consistency)
    )
    summary = {
        "enabled": True,
        "min_views": int(args.multiview_min_views),
        "foreign_consistency": float(args.multiview_foreign_consistency),
        "voted_point_count": int((visible_count > 0).sum()),
        "foreign_point_count": int(foreign_points.sum()),
    }
    return foreign_points, consistency, visible_count, summary


def multiview_negative_points(
    points_xyz: np.ndarray,
    foreign_point_mask: np.ndarray,
    camera: dict,
    image_shape: tuple[int, int],
    ...
    """把当前视角可见且落入第一遍 mask 的一致性异物点采样为 SAM 硬负点。"""
    if not foreign_point_mask.any() or args.multiview_negative_points <= 0:
    ...
    candidates = foreign_point_mask & visible
    candidates &= (u >= x0) & (u <= x1) & (v >= y0) & (v <= y1)
    ids = np.flatnonzero(candidates)
    if len(ids) == 0:
        return np.zeros((0, 2), dtype=np.float32)
    ids = ids[first_mask[v[ids], u[ids]]]
    if len(ids) == 0:
        return np.zeros((0, 2), dtype=np.float32)
    points_2d = np.unique(np.stack([u[ids], v[ids]], axis=1), axis=0)
    return sample_points(points_2d, args.multiview_negative_points)
...
def parse_args():
    ...
    parser.add_argument("--mode", choices=("sam_points", "grounded_sam", "hybrid"), default="hybrid")
    ...
    parser.add_argument("--multiview-consistency", action="store_true", help="用第一遍 mask 的3D多视角一致性生成硬负点并二次细化 SAM")
    parser.add_argument("--no-multiview-consistency", action="store_false", dest="multiview_consistency")
    parser.add_argument("--multiview-min-views", type=int, default=3, help="一个场景点参与一致性判断所需的最少可见视角")
    parser.add_argument("--multiview-foreign-consistency", type=float, default=0.5, help="非目标点落入第一遍 mask 的最低视角比例")
    parser.add_argument("--multiview-negative-points", type=int, default=24, help="每个视角加入第二遍 SAM 的多视角硬负点数")
    parser.add_argument("--multiview-max-negative-cover", type=float, default=0.15, help="第二遍 mask 允许覆盖硬负点的最大比例")
    parser.add_argument("--multiview-depth-tolerance", type=float, default=0.03, help="多视角投票与硬负点投影的深度可见容差")
    ...
    parser.set_defaults(constrain_to_3d_support=False, multiview_consistency=True)
...
def main():
    ...
    ply_out = output_dir / f"{safe_class}_filted.ply"
    ...
    first_pass_views = []
    for index, candidate in enumerate(tqdm(view_candidates, desc="SAM 第一遍")):
        ...
        first_pass_views.append(
            {
                **candidate,
                "first_mask": best[1],
                "first_stats": best[2],
                "box": np.asarray(best[2]["box"], dtype=np.float32),
                "hull_mask": hull_mask,
                "support_mask": support_mask,
                "pos_points": pos_points,
                "base_neg_points": neg_points,
            }
        )

    if not first_pass_views:
        raise RuntimeError(f"第一遍 SAM 没有生成任何有效视角，skipped={skipped}")

    multiview_active = bool(args.multiview_consistency and args.mode == "sam_points" and len(first_pass_views) >= 2)
    foreign_point_mask = np.zeros(len(points_xyz), dtype=bool)
    multiview_summary = {
        "enabled": multiview_active,
        "requested": bool(args.multiview_consistency),
        "reason": None,
    }
    if multiview_active:
        foreign_point_mask, _, _, multiview_summary = build_multiview_consistency(
            points_xyz,
            point_mask,
            first_pass_views,
            args,
        )
        print(f"多视角一致性: 找到 {int(foreign_point_mask.sum())} 个稳定非目标点")
    elif args.multiview_consistency and args.mode != "sam_points":
        multiview_summary["reason"] = "仅在 sam_points 模式启用"
    elif args.multiview_consistency:
        multiview_summary["reason"] = "有效视角少于2张"

    for view in tqdm(first_pass_views, desc="SAM 多视角细化与保存"):
        stem = view["stem"]
        ...
        if image_bgr is None:
            ...
            continue
        image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
        points_2d = view["points_2d"]
        final_mask = view["first_mask"]
        stats = view["first_stats"]
        hard_neg_points = np.zeros((0, 2), dtype=np.float32)
        refinement = {
            "enabled": multiview_active,
            "used_second_pass": False,
            "hard_negative_points": 0,
        }

        if multiview_active and foreign_point_mask.any():
            hard_neg_points = multiview_negative_points(
                points_xyz,
                foreign_point_mask,
                view["camera"],
                view["image_shape"],
            ...
            refinement["hard_negative_points"] = int(len(hard_neg_points))
            if len(hard_neg_points):
                refined_neg_points = np.concatenate([view["base_neg_points"], hard_neg_points], axis=0)
                refined_mask, refined_stats = choose_sam_mask(
                    sam_predictor,
                    image_rgb,
                    view["box"],
                    view["pos_points"],
                ...
                hx = np.clip(hard_neg_points[:, 0].astype(int), 0, refined_mask.shape[1] - 1)
                hy = np.clip(hard_neg_points[:, 1].astype(int), 0, refined_mask.shape[0] - 1)
                hard_negative_cover = float(refined_mask[hy, hx].mean())
                refined_ok = (
                    refined_stats["mask_area"] >= args.min_mask_pixels
                    and refined_stats["point_cover"] >= args.min_point_cover
                    and refined_stats["area_ratio"] <= args.max_area_ratio
                    and hard_negative_cover <= args.multiview_max_negative_cover
                )
                refinement["second_pass_accepted"] = bool(refined_ok)
                refinement["hard_negative_cover"] = hard_negative_cover
                refinement["second_pass_stats"] = refined_stats
                if refined_ok:
                    final_mask = refined_mask
                    stats = refined_stats
                    stats["box"] = view["box"].tolist()
                    stats["visible_points"] = int(len(points_2d))
                    stats["visible_ratio"] = float(view["visible_ratio"])
                    stats["view_score"] = float(view["view_score"])
                    stats["support_pixels"] = int(view["support_mask"].sum())
                    stats["negative_points"] = int(len(refined_neg_points))
                    stats["accepted"] = True
                    refinement["used_second_pass"] = True

        stats["multiview_refinement"] = refinement
        if args.constrain_to_3d_support:
            final_mask = np.logical_and(final_mask, view["support_mask"])
        ...
        selected_views.append(
            {
                ...
                "view_prefilter": view["view_stats"],
                "sam_stats": stats,
            }
        )
    ...
    summary = {
        ...
        "mode": args.mode,
        "multiview_consistency": multiview_summary,
